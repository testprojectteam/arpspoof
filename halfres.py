...
def func(raw_data):
    ...
    #print len(p)
                ...
                sniffer_thread = threading.Thread(target=sniftcp, name='sniffer_thread')
                sniffer_thread.start()
                send_thread = threading.Thread(target=send_packets, name='packet_thread')
                send_thread.start()
                global p
                try:
                    sniftcp()
...
def snif():
    ...
    while True:
        ...
        func_thread = threading.Thread(target=func, name='func_thread', args = (raw_data,))
        func_thread.start()
    return raw_data
